...
export default function App() {
  ...
  const processedBookings = useMemo(() => {
    ...
    // Sort order
    result.sort((a, b) => {
      const dateA = new Date(a.created_at || `${a.booking_date || ''} ${a.booking_time || ''}`).getTime() || 0;
      const dateB = new Date(b.created_at || `${b.booking_date || ''} ${b.booking_time || ''}`).getTime() || 0;
                          ...
                          {paginatedBookings.map((b) => {
                            ...
                            return (
                              ...
                              <td className="px-5 py-4 min-w-[150px]">
                                <div className="font-semibold text-slate-900">{b.patient_name || 'Unknown'}</div>
                                <div className="text-xs text-slate-500 font-medium flex items-center gap-1 mt-0.5">
                                  <Phone size={12} className="text-slate-400" />
                                  <span>{b.contact_number || 'N/A'}</span>
                                </div>
                              </td>
